-- Garante que, quando uma sobra publicada bater com uma busca automatica,
-- tanto quem procura quanto quem anunciou a sobra recebam notificacao.
  ...
  LOOP
    ...
    IF vendedor_uid IS NOT NULL AND vendedor_uid <> ped.user_id THEN
      ...
      VALUES (
        vendedor_uid,
        'match_vendedor',
        'Existe um comprador procurando sua sobra',
        format('%s - %s - %smm - precisa de %sx%scm%s',
          COALESCE(ped.fabricante, '-'), ped.padrao, ped.espessura_mm,
          ped.comprimento_min_cm, ped.largura_min_cm,
          CASE WHEN dist IS NOT NULL THEN ' - ' || round(dist::numeric, 1) || ' km' ELSE '' END),
        NEW.id,
        ped.id
      );
