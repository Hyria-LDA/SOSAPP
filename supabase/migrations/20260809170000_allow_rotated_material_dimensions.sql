-- Permite que uma busca aceite a mesma peca girada em 90 graus.
-- Exemplo: uma busca 200x50 tambem aceita um material 50x200.

ALTER TABLE public.pedidos_material
  ADD COLUMN IF NOT EXISTS medidas_invertiveis boolean NOT NULL DEFAULT false;
...
BEGIN
  ...
  FOR ped IN
    ...
    WHERE status = 'ativo'
      ...
      AND (
        (
          NEW.comprimento_cm >= comprimento_min_cm
...
BEGIN
  ...
  FOR mat IN
    ...
    WHERE m.status = 'ativo'
      ...
      AND (
        (
          m.comprimento_cm >= NEW.comprimento_min_cm
